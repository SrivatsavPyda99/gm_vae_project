...
def save_checkpoint(gpu_is_available, images_remade, vae, num_gen, base_dir, save_iter, elbo_log, logpx_log, logpz_log, logqz_log):
    """
    ...
    np.save(os.path.join(base_dir, "elbo_log.npy"), elbo_log)
    np.save(os.path.join(base_dir, "logpx_log.npy"), logpx_log)
    np.save(os.path.join(base_dir, "logpz_log.npy"), logpz_log)
    np.save(os.path.join(base_dir, "logqz_log.npy"), logqz_log)
...
def main(args):
    ...
    for t in range(max_iterations):
        ...
        if gpu_is_available:
            elbo_log.append(elbo.data.cpu().numpy())
            logpx_log.append(logpx.data.cpu().numpy())
            logpz_log.append(logpz.data.cpu().numpy())
            logqz_log.append(logqz.data.cpu().numpy())
            #mu_log.append(mu.data.cpu().numpy())
            #logvar_log.append(logvar.data.cpu().numpy())
        else:
            elbo_log.append(elbo.data.numpy())
            logpx_log.append(logpx.data.numpy())
            logpz_log.append(logpz.data.numpy())
            logqz_log.append(logqz.data.numpy())
            #mu_log.append(mu.data.numpy())
            #logvar_log.append(logvar.data.numpy())
        
        running_value += elbo.data
        ...
        if (it+1) % print_interval == 0:
            ...
            print("######################")
            print("step {}/{}. average elbo value of last {} steps: {}".format(it+1, max_iterations, print_interval,
        ...
        if (it+1) % save_interval == 0:
            save_checkpoint(gpu_is_available, images_remade, vae, num_gen, save_dir, it+1, elbo_log, logpx_log, logpz_log, logqz_log)
            loss_log = []
            elbo_log = []
            logpx_log = []
            logpz_log = []
